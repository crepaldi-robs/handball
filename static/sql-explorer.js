...
if (sqlRoot) {
  ...
  function renderResult(payload) {
    results.replaceChildren(); const table = document.createElement("table"); const head = document.createElement("tr");
  ...
  async function run(endpoint = "query") { const payload = await request(`/api/v1/sql/${endpoint}`, { method: "POST", body: JSON.stringify({ sql: editor.value, page: 1, page_size: 100 }) }); if (endpoint === "explain") { renderResult({ columns: ["id", "parent", "notused", "detail"], rows: payload.items, has_more: false }); } else renderResult(payload); }
